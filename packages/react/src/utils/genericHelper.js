  ...
  for (let i = 0; i < outgoing; i += 1) {
    if (nodeData.outputs && nodeData.outputs.length) {
      ...
      if (nodeData.outputs.length > 1) {
        ...
        let tempOutput = nodeData.outputs[0];
        const newOutput = {
          id: `${newNodeId}-output-${
            nodeData.name
          }-${tempOutput.baseClasses.join("|")}`,
          name: tempOutput.name,
          label: tempOutput.label,
          description: tempOutput.description ?? "",
          type: tempOutput.baseClasses.join("|"),
        };
        outputAnchors.push(newOutput);
      }
    } else {
